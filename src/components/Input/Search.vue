      ...
      <el-col :span="layout === 'right' ? 22 : 24">
        ...
        >
            ...
            <template v-if="item.itemType === 'radio'">
              ...
              >
                <template v-if="item.radioType === 'radio'">
                  ...
                  >{{ item.optionLabel ? v[item.optionLabel] : v.label }}</el-radio>
                </template>
                <template v-else-if="item.radioType === 'button'">
                  <el-radio-button
                    ...
                    v-bind="{ ...getItemBindValue(item) }"
                    :label="item.optionValue ? v[item.optionValue] : v.value"
                  >{{ item.optionLabel ? v[item.optionLabel] : v.label }}</el-radio-button>
                </template>
              </el-radio-group>
            </template>
          ...
          <el-form-item v-if="data.length > 0 && layout === 'classic'">
            ...
            <el-button v-if="showReset" icon="el-icon-refresh-right" @click="resetForm">重置</el-button>
          </el-form-item>
        </el-form>
      </el-col>
